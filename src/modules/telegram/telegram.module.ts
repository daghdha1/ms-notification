...
export class TelegramModule implements OnModuleInit {
  ...
  async onModuleInit() {
    if (convertEnvToBoolean(process.env.KAFKA_ACTIVE)) {
      console.log(
        '\x1b[33m%s\x1b[0m',
        `${Provider.KafkaConsumer} client is connecting`,
      );
      await this.kafkaClient.connect();
      // only with request-response message pattern over kafka
      // this.kafkaClient1.subscribeToResponseOf(process.env.KAFKA_AUTH_JWT_TOPIC);
      console.log(
        '\x1b[32m%s\x1b[0m',
        `${Provider.KafkaConsumer} client connected`,
      );
    }
  }
}
