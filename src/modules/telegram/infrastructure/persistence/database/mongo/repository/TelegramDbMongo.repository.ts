...
export class TelegramDbMongoRepository extends MongoRepository implements TelegramDbRepository {
  constructor(
  ...
  public async saveDhlTrackingEvent(event: any): Promise<boolean> {
    const db = (await this.pool.connect()).db(TelegramConstants.MONGO_NOTIFICATION_TELEGRAM_DB)
    const cursor = await db.collection(TelegramConstants.MONGO_NOTIFICATION_TELEGRAM_COL).insertOne(event)
    if (!cursor.acknowledged) throw new TelegramException('TelegramMongoRepository error')
    return true
  }
}
